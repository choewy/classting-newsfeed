...
describe('SchoolService', () => {
  ...
  beforeAll(async () => {
    module = await Test.createTestingModule({
      providers: [Mock.Repository(SchoolRepository), SchoolService],
    }).compile();
  ...
  describe('createSchool', () => {
    it('이미 학교 페이지를 생성한 경우 AlreadyHasSchoolException를 던진다.', () => {
      const admin = SchoolFixture.Admin({ school: { id: 1 } });
      const command = SchoolFixture.CreateSchoolCommand();

      expect(service.createSchool(admin, command)).rejects.toBeInstanceOf(AlreadyHasSchoolException);
    });
    ...
    it('같은 이름, 같은 지역의 학교가 이미 존재하는 경우 AlreadyExistSchoolException를 던진다.', () => {
      ...
      const admin = SchoolFixture.Admin();
      const command = SchoolFixture.CreateSchoolCommand();

      expect(service.createSchool(admin, command)).rejects.toBeInstanceOf(AlreadyExistSchoolException);
    });
    ...
    it('학교 페이지가 생성되면 SchoolDto를 반환한다.', () => {
      ...
      expect(service.createSchool(admin, command)).resolves.toBeInstanceOf(SchoolDto);
    });
  });
});
